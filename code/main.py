...
if __name__ == '__main__':
    ...
    _, test_list, _, _ = dataset_AES.leaveone_split('AES-Detrust01')
    ...
    model = ModelA(INPUT_DIM, HIDDEN_DIM, NUM_CLASSES).to(DEVICE)
    test_model = torch.load('model.pth')
    model.eval()
    with torch.no_grad():
        logits = model(test_normalize_adjacency, test_node_features, None)
        test_logits = logits
        test_acc = torch.eq(
